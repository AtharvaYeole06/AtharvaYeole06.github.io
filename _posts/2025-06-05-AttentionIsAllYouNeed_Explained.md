...
## Positional encoding, how does it know order?

Attention is a set operation. It looks at all tokens simultaneously with no sense of order.
The model would have no idea which word came first.
The fix to this issue is before tokens enter the Transformer, you add a positional encoding
vector to each token's embedding.
The paper uses sine and cosine waves.

PE(pos, 2i)   = sin(pos / 10000^(2i / d_model))
PE(pos, 2i+1) = cos(pos / 10000^(2i / d_model))

Every position in the sequence gets a unique vector generated by mixing sine and cosine waves at many different frequencies.
This gets added directly to the token embedding. So token at position 3 has a slightly different input vector than same token at position 7.
